...
/** Private class tokens for `src/components/ui` primitives. Pages import components, not this object. */
export const ui = {
	muted: 'text-muted',
	panel:
	...
	field: 'grid gap-[0.35rem] text-[0.9rem] text-muted [&_input]:text-base [&_select]:text-base [&_[data-select]]:text-base [&_textarea]:text-base',
	...
	snackHost:
		'snack-host fixed z-[60] inset-x-0 bottom-[calc(1.15rem+env(safe-area-inset-bottom,0px))] flex flex-col-reverse items-center gap-[0.45rem] px-4 pointer-events-none max-sm:bottom-[calc(5.1rem+env(safe-area-inset-bottom,0px))] max-sm:px-3',
	snack:
	...
	stickyActions:
		'sticky-actions sticky z-20 bottom-[calc(0.75rem+env(safe-area-inset-bottom,0px))] flex !flex-nowrap items-center justify-end gap-3 p-3 px-4 border border-line rounded-2xl bg-canvas/92 backdrop-blur-[10px] shadow-lift max-sm:bottom-[1.5rem] max-sm:gap-[0.55rem] max-sm:[&>a]:w-auto max-sm:[&>button]:w-auto max-sm:[&>a]:flex-none max-sm:[&>button]:flex-none',
	...
	segItemFill:
		'appearance-none flex flex-1 min-w-0 items-center justify-center gap-1 relative min-h-11 px-1 py-2 border-0 border-r border-line last:border-r-0 rounded-none bg-transparent text-muted text-center text-[0.75rem] leading-none font-inherit no-underline cursor-pointer whitespace-nowrap overflow-hidden transition-[color,background-color,border-color] duration-150 ease-out sm:gap-1.5 sm:px-[0.85rem] sm:text-[0.9rem] aria-[pressed=false]:hover:text-fg aria-[pressed=false]:hover:bg-[color-mix(in_srgb,var(--accent)_18%,transparent)] active:text-fg aria-[pressed=true]:text-accent-ink! aria-[pressed=true]:bg-accent! aria-[pressed=true]:font-semibold aria-[pressed=true]:active:text-accent-ink'
} as const;
