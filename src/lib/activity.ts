...
export type ActivityType = 'run' | 'walk' | 'ride' | 'swim';
export const ACTIVITY_TYPES: ActivityType[] = ['run', 'walk', 'ride', 'swim'];
...
export function normalizeActivityType(v: string | null | undefined): ActivityType {
	...
	if (['ride', 'bike', 'biking', 'cycling', 'bicycle', 'cycle', 'ebikeride'].includes(t))
		return 'ride';
	if (['swim', 'swimming', 'openwaterswim', 'lap_swimming'].includes(t)) return 'swim';
	return 'run';
}
