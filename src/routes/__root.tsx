...
function RootShell() {
	...
	return (
		<RootDocument>
			...
			<ScrollRestore />
			<ScrollToTop />
			<div className="app-shell relative z-1 flex flex-1 flex-col w-[min(1120px,calc(100%-2rem))] min-h-dvh mx-auto pt-5 pr-[env(safe-area-inset-right,0px)] pb-[calc(4rem+env(safe-area-inset-bottom,0px))] pl-[env(safe-area-inset-left,0px)] max-sm:w-full max-sm:h-dvh max-sm:min-h-0 max-sm:overflow-hidden max-sm:pt-0 max-sm:pb-0 max-sm:pl-0 max-sm:pr-0">
				<header className="app-header flex flex-none flex-wrap items-center justify-between gap-x-4 gap-y-2 mb-8 max-sm:mb-0 sm:pt-[calc(0.85rem+env(safe-area-inset-top,0px))] sm:pb-[0.85rem] sm:border-b sm:border-line">
					<Link
						to="/"
